...
Last updated: 2026-05-07 (Slice 10 follow-up - pod AI evaluation v2)
...
Slice 10 is open with the core authoritative AI contract shipped for both decks and pods. Deck prompt `deck-eval-v2` embeds the long-form Commander Brackets rules text, the full Game Changers list, the current banlist, the six-axis power-band rubric, and the deck name + decklist; the LLM produces the bracket call, sub-band, and six 0-10 axes itself, validated by `Codex::DeckEvaluationValidator` against schema `deck-evaluation-v2`. Pod prompt `pod-eval-v2` sends the same bracket briefing plus every seated decklist and deterministic pod snapshot; the LLM produces the authoritative pod bracket spread, pod-level axes, per-deck table roles, and paste-ready Rule 0 brief, validated by `Codex::PodEvaluationValidator` against schema `pod-evaluation-v2`. Deck and pod show pages collapse deterministic reads under preliminary disclosures once a successful v2 run exists, and both pages expose a one-click "Connect ChatGPT / Codex account" CTA when no Codex account is linked. Remaining Slice 10 work is deliberately about secondary surfaces: public deck sharing and analysis exports.
