...
Last updated: 2026-05-04 (Phase 4 first tranche: provider adapter interface and pasted decklist import)
...
- A pluggable provider adapter interface lives at `Decks::Adapters::Base` with a shared `ParsedDeck` value object and `parse`/`fetch`/`refresh` extension points. The first concrete adapter, `Decks::Adapters::PastedText`, wraps `Decks::TextDecklistParser` and stamps `source_type` plus byte-size/line-count `source_metadata` on every parse. `Decks::Importer` consumes adapter output, applies an optional `commander_hint`, persists `Deck`/`Commander`/`DeckCard` rows in a transaction, derives a deck name from the supplied label or the commander, records source attribution and any unparsed lines in `import_metadata`, and validates that imports include a commander and at least one parsed card line. `DecksController` (`/decks`, `/decks/new`, `/decks/:id`, with `destroy`) is rate-limited (30/5min), scopes lookups to the signed-in user, surfaces unparsed lines on the deck show page, links from the dashboard, and records `deck.imported`/`deck.removed` audit events. A `DeckImportForm` ActiveModel form caps decklist payloads at 64 KB and enforces a non-empty content line check before the importer runs. Service unit tests, controller integration tests, and a Rack::Test system test cover the happy path, missing-commander rejection, blank-input rejection, unparsed-line surfacing, source attribution, and tenant isolation.
...
- [~] Phase 4 - Build deck import and provider adapters. (Provider adapter interface + pasted decklist import are live; text-file upload, Archidekt/Moxfield URL adapters, refresh jobs, revision history, exports, and provider fixtures still pending.)
...
- [~] Adapter unit tests. (Pasted-text adapter and `Decks::Importer` are covered by service tests; provider URL adapters land later.)
- [ ] Fixture-based provider parser tests.
- [~] Browser import tests. (Rack::Test system coverage for paste-import happy path and blank-input rejection; provider URL flows land later.)
