...
Last updated: 2026-05-04 (Phase 4 second tranche: text file decklist upload import)
...
- A pluggable provider adapter interface lives at `Decks::Adapters::Base` with a shared `ParsedDeck` value object and `parse`/`fetch`/`refresh` extension points. The first concrete adapter, `Decks::Adapters::PastedText`, wraps `Decks::TextDecklistParser` and stamps `source_type` plus byte-size/line-count `source_metadata` on every parse. The second adapter, `Decks::Adapters::TextFile`, accepts an uploaded file (`ActionDispatch::Http::UploadedFile`-shaped object), enforces a 64 KB byte-size cap, validates extensions (`.txt`, `.text`, `.dec`, `.deck`, `.cod`, `.md`, `.markdown`, `.csv`) and `text/*` content types, requires UTF-8 decodable bytes, strips a leading UTF-8 BOM, and stamps filename, content type, and uploaded byte size into `source_metadata` while sharing the same parser as the pasted-text adapter. `Decks::Importer` exposes both `import_pasted_text` and `import_text_file` helpers, consumes adapter output through a single `import` path, applies an optional `commander_hint`, persists `Deck`/`Commander`/`DeckCard` rows in a transaction, derives a deck name from the supplied label or the commander, records source attribution and any unparsed lines in `import_metadata`, validates that imports include a commander and at least one parsed card line, and surfaces `Decks::Adapters::TextFile::InvalidFile` errors back to the caller as result error messages. `DecksController` (`/decks`, `/decks/new`, `/decks/:id`, with `destroy`) is rate-limited (30/5min), scopes lookups to the signed-in user, accepts either a pasted decklist or a `decklist_file` upload (uploaded file takes precedence when both are present), surfaces unparsed lines on the deck show page, links from the dashboard, and records `deck.imported`/`deck.removed` audit events. A `DeckImportForm` ActiveModel form caps both pasted decklist payloads and uploaded file sizes at 64 KB, requires either pasted text or a non-empty upload, and enforces a non-empty content line check before the importer runs. Service unit tests, adapter unit tests, controller integration tests, and Rack::Test system tests cover the happy paste path, the happy upload path, missing-commander rejection, blank-input rejection, unsupported file extensions, oversize/non-UTF-8 file payloads, unparsed-line surfacing, source attribution, and tenant isolation.
...
- [~] Phase 4 - Build deck import and provider adapters. (Provider adapter interface, pasted decklist import, and text-file upload import are live; Archidekt/Moxfield URL adapters, refresh jobs, revision history, exports, and provider fixtures still pending.)
...
- [~] Adapter unit tests. (Pasted-text adapter, text-file adapter, and `Decks::Importer` are covered by service tests; provider URL adapters land later.)
- [ ] Fixture-based provider parser tests.
- [~] Browser import tests. (Rack::Test system coverage for paste-import and text-file upload happy paths plus blank-input rejection; provider URL flows land later.)
