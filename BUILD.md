...
Last updated: 2026-05-07 (Slice 10 closed - AI-authoritative public deck shares + AI-aware analysis exports)
...
- **Codex account-auth:** encrypted credential storage, device-code login as the production-safe hosted path, browser login labeled as same-machine-only, refresh-status, remote sign-out, per-user/global quota policy with rate-limit display. The live Codex App Server JSON-RPC transport is wired behind `CODEX_APP_SERVER_ENABLED`; default remains fail-closed with `NullTransport`. The deck and pod show pages surface a "Connect ChatGPT / Codex account" CTA when no account is linked so the path to AI evaluation is one click from the evaluated object. Single-deck AI evaluation runs on prompt `deck-eval-v2` and schema `deck-evaluation-v2` — the LLM is fed the long-form bracket rules text, the canonical Game Changers list, and the current Commander banlist, then asked to make the bracket call and the six 0-10 axis calls itself; once a v2 run succeeds it becomes the authoritative deck evaluation on the show page (deterministic stays as a collapsible preliminary read). Pod AI evaluation runs on prompt `pod-eval-v2` and schema `pod-evaluation-v2`; once it succeeds it becomes authoritative for pod bracket spread, pod-level axes, per-deck table roles, and the Rule 0 brief (deterministic pod analysis stays collapsed as a preliminary read).
...
- **Pods (2–4 decks):** build a pod from your own decks, optionally including one guest deck via pasted decklist or public Archidekt / Moxfield URL. Pods get per-axis spread/average/outliers, archenemy/pubstomp/durdle warnings, a Rule 0 brief (power band, tempo, combo/stax notes, salt/friction notes), and per-deck swap suggestions. Mobile-readable and printable show page. Opt-in revocable public share link with one-click copy. The public pod surface mirrors the AI evaluation when one is present. Guest decks live only with the pod and are removed when the pod is removed.
...
Slice 10 is closed. The deck-evaluation v2 contract is wired through every deck surface: signed-in deck show, signed-in analysis exports, public token-gated `/d/:token` share page, and public token-gated analysis exports. When a deck has a successful AI run, AI is authoritative for bracket call, sub-band, the six 0-10 axes, friction drivers, talking points, and recommendations; deterministic stays as a collapsed preliminary read on every surface. Pod prompt `pod-eval-v2` covers parallel ground for pods and is mirrored on the public pod surface. The shared public surface adds a one-click copy-link control, AI-aware OG meta description, and a clearer "what's public, what stays private" disclosure. Public surface still deliberately omits matchup notes, table performance, collection fit, opponent identity, and audit history — those stay private regardless of share state.
...
Single-deck AI evaluation uses prompt `deck-eval-v2` (`Codex::DeckEvaluationPrompt`) and response schema `deck-evaluation-v2` (`Codex::DeckEvaluationSchema` + `Codex::DeckEvaluationValidator`). The prompt is built from `Codex::BracketBriefing`, which embeds the Commander Brackets long-form rules text, the canonical Game Changers list (loaded from `db/seeds/commander/brackets/game_changers.json`), the Commander banlist (from `db/seeds/commander/legality_snapshots/current.json`), and the six-axis 0-10 power-band rubric. The LLM is asked to make the bracket call, sub-band call, and the six 0-10 axis calls itself; the deterministic feature vector is included only as a sanity check. Real-world LLM runs drift toward a flatter natural shape (bracket fields hoisted to the root, axis `uncertainty` omitted, an extra `legality_review.legal` flag, prompt-input keys echoed back into the response), so `Codex::DeckEvaluationNormalizer` reshapes the response into the canonical layout before validation: it hoists `restrictions`/`game_changers`/`evidence`/`uncertainty` into `bracket`, drops echoed `deck_id`/`deck_name`/`format`, maps `bracket.rationale` into `headline`/`tagline`, defaults missing axis `uncertainty` arrays, strips `legality_review.legal`, coerces unknown restriction statuses to a safe value, and synthesizes a summary from the bracket headline/tagline when the AI omits it. `friction_drivers`, `rule_zero_talking_points`, and `recommendations` are now optional at the root and default to `[]` after normalization. The prompt's system message and `response_contract.skeleton` describe the canonical shape explicitly so new runs land on it directly. `Decks::AiEvaluationPresenter.for(latest_ai_run)` returns the AI bracket, sub-band, restrictions, axes, friction drivers, talking points, and recommendations to the deck show page when a v2 run has succeeded — the `decks/_ai_analysis.html.erb` partial renders that as the canonical deck evaluation, while the existing deterministic `decks/_analysis.html.erb` surface is collapsed under a "preliminary deterministic read" disclosure. Legality remains source-backed deterministic; the AI is only allowed to flag legality concerns under an optional `legality_review` block, never to override the rules engine.

The public deck share surface mirrors the AI-authoritative read. `PublicDecksController#show` resolves `Decks::AiEvaluationPresenter.for(@deck.latest_ai_run)` alongside the deterministic run; the `public_decks/show` view renders the shared `decks/_ai_analysis` partial when an AI evaluation is present and tucks the deterministic numbers under a `Show preliminary deterministic read` disclosure. The public-surface privacy boundary still excludes matchup notes, table performance, collection fit, opponent identity, and audit history — only the deck name, commander, decklist, deterministic analysis, and (when present) the deck-evaluation-v2 AI verdict are exposed. The public pod surface gets the parallel treatment via `PublicPodsController` + `pods/_ai_analysis`. The signed-in deck and pod share boxes plus the public deck/pod pages all surface a Stimulus-driven `copy-link` button that reads from `data-copy-link-url-value`. `Decks::AnalysisExporter` bumped to schema v2: JSON now carries an `ai_evaluation` block (with `authoritative: true`, prompt/model/run metadata, bracket, axes, drivers, talking points, recommendations, and any optional `legality_review`) when a successful v2 run exists; Markdown leads with an "AI evaluation (Commander Brackets)" section and collapses the deterministic numbers under a `<details>` block. Both signed-in and public token-gated analysis exports go through the same exporter, so anyone with a share link can pull the AI verdict in either format.

Pod AI evaluation now uses prompt `pod-eval-v2` (`Codex::PodEvaluationPrompt`) and response schema `pod-evaluation-v2` (`Codex::PodEvaluationSchema` + `Codex::PodEvaluationValidator`). The prompt includes the same bracket briefing plus each seated deck's commander/main decklist, deterministic bracket/six-axis snapshot, pod aggregate snapshot, warnings, suggestions, and deterministic Rule 0 brief. `Codex::EvaluationRunner` selects `PodEvaluationValidator` for pod targets and stores the validated v2 response on the `AnalysisRun`. `Pods::AiEvaluationPresenter.for(latest_ai_run)` returns the AI pod bracket spread, Rule 0 brief, pod-level axes, per-deck table roles, friction drivers, and recommendations to the pod show page; `pods/_ai_analysis.html.erb` renders it as the canonical pod table read and collapses `pods/_analysis.html.erb` under a "preliminary deterministic pod read" disclosure. The shared `codex/_ai_evaluation` CTA now receives the current user's Codex account on pod pages too, so connected users see the run button and disconnected users see the connect path.

Ideal Magic ships an installable PWA: `/manifest.json` and `/service-worker.js` are served from `app/views/pwa/`, the layout links the manifest, declares `theme-color`, `apple-mobile-web-app-*` metadata and a maskable icon, and a Stimulus controller registers the worker, listens for waiting updates, and surfaces a reload banner so users are not trapped on stale assets. The service worker keeps the app shell (root, manifest, icon, `/assets/*`) cache-first with background revalidation, network-firsts the allowlisted `/app`, `/decks`, `/pods`, and `/sessions` navigations into a cache so a recently visited page renders offline, and is keyed by `Rails.application.config.assets.version` plus the app revision so deploys evict old caches. Authenticated phone-sized viewports render a fixed five-up bottom nav (Decks, Pods, Sessions, Journal, Collection) keyed off `ApplicationController#mobile_nav_section`. The deck index renders a search box plus bracket and status filters that compose against `current_user.decks`. A global `offline` Stimulus controller listens for `online`/`offline` events, exposes a top-of-viewport "anything you submit is paused" banner, and disables `[type=submit]` controls inside any `form[data-offline-disable]` — wired into deck import, AI evaluation, pod creation, session creation/seating/result submission, matchup-note CRUD, collection add, and collection import forms so the UI never claims a server-side write happened while disconnected. Deck show pages now expose plain-text/CSV/JSON decklist downloads and Markdown/JSON deterministic-analysis downloads (`Decks::Exporter`, `Decks::AnalysisExporter`, served at `GET /decks/:id/export.{text,csv,json}` and `GET /decks/:id/analysis.{markdown,json}`). Decks also carry opt-in revocable share tokens (`decks.share_token` / `shared_at` / `share_revoked_at`, mirrored from the pod model) issued and revoked through `DeckSharesController`, surfaced on the deck show page, and consumed by `PublicDecksController` at `GET /d/:token`, `GET /d/:token/export.{text,csv,json}`, and `GET /d/:token/analysis.{markdown,json}` — the public surface renders the decklist, commanders, deterministic six-axis analysis, bracket placement, legality, and tuning suggestions, but never the AI run, table-performance summary, collection fit, matchup notes, audit history, or any other playgroup-tied data. The signed-in account export (`Accounts::Exporter`, schema v2) ships full account state — account/codex/provider links plus decks, analysis runs, pods, collection cards + import history, game nights with players/decks/seats/results, matchup notes, and audit events. Codex-backed deck and pod AI evaluations remain queue-able from the show pages, gate through the existing quota policy, persist prompt/input/output/model/latency/rate-limit replay metadata on `AnalysisRun`, and render validated authoritative deck/pod reads beside collapsed deterministic evidence. The default Codex transport still fails closed unless `CODEX_APP_SERVER_ENABLED=true` is configured.
...
- [x] `Codex::DeckEvaluationPrompt` rewritten to prompt-version `deck-eval-v2` — feeds bracket briefing + deck name + decklist + deterministic signals + response contract; `Codex::EvaluationRunner` validates deck runs with the deck v2 validator.
...
- [x] Pod evaluation prompt + schema upgrade to v2-equivalent so AI is also authoritative for pod bracket spread and Rule 0 brief. (`Codex::PodEvaluationPrompt` now emits prompt `pod-eval-v2` against `Codex::PodEvaluationSchema` / `Codex::PodEvaluationValidator`; it sends full seated decklists, bracket briefing, deterministic pod snapshots, warnings, suggestions, and the deterministic Rule 0 brief. `Codex::EvaluationRunner` validates pod runs with the pod v2 validator, and the pod show page renders `Pods::AiEvaluationPresenter` + `pods/_ai_analysis.html.erb` as the canonical pod read once a run succeeds.)
- [x] Public deck share page mirrors AI evaluation when a deck has a successful AI run. (`PublicDecksController#show` now resolves `Decks::AiEvaluationPresenter.for(@deck.latest_ai_run)`; `app/views/public_decks/show.html.erb` renders the shared `decks/_ai_analysis` partial as the canonical read when present and collapses the deterministic preliminary read under a disclosure. Public surface still deliberately omits matchup notes, table performance, collection fit, and audit history. Public pod surface gets the parallel treatment via `PublicPodsController` + `pods/_ai_analysis`.)
- [x] AI-aware analysis exports (Markdown/JSON include AI bracket + axes when present). (`Decks::AnalysisExporter` bumped to schema v2 — JSON now ships an `ai_evaluation` block when a successful AI run exists, Markdown leads with the AI Commander Brackets section and collapses the deterministic numbers under a `<details>` block. Both signed-in and token-gated public exports go through the same exporter.)
...
- 2026-05-07 — Slice 10 closed: AI-authoritative public deck shares + AI-aware analysis exports. Public `/d/:token` page now resolves the latest AI run through `Decks::AiEvaluationPresenter` and renders the shared `decks/_ai_analysis` partial as the canonical read with deterministic collapsed under "Show preliminary deterministic read"; the public pod surface gets the parallel treatment via `PublicPodsController` + `pods/_ai_analysis`. `Decks::AnalysisExporter` bumped to schema v2: JSON now ships an `ai_evaluation` block when a successful v2 run exists, Markdown leads with the AI Commander Brackets section and collapses deterministic under `<details>`; signed-in and token-gated public analysis exports both go through the same path. Public sharing UX got a Stimulus `copy-link` controller for one-click copy on signed-in deck/pod share boxes plus the public deck/pod pages, AI-aware OG `meta_description` on the public pages, an explicit "what's public, what stays private" disclosure, and a "Try Ideal Magic" CTA. Public surface still deliberately omits matchup notes, table performance, collection fit, opponent identity, and audit history.
- 2026-05-07 — Slice 10 pod v2: pod AI evaluation now runs prompt `pod-eval-v2` against schema `pod-evaluation-v2`. The prompt sends full seated decklists, bracket briefing, deterministic pod aggregate/warning/Rule 0 context, and asks Codex for the authoritative pod bracket spread, pod-level axes, per-deck table roles, and paste-ready Rule 0 brief. `EvaluationRunner` validates pod runs with `Codex::PodEvaluationValidator`; pod pages render the successful v2 run as the canonical table read and collapse the deterministic pod analysis as preliminary. This is a one-box pass because the next unchecked Slice 10 item changes the public sharing/export surface rather than the pod evaluation contract.
- 2026-05-07 — Slice 10 follow-up: AI deck evaluation now tolerates the LLM's natural response shape. `Codex::DeckEvaluationNormalizer` reshapes incoming responses before validation — hoists `restrictions`/`game_changers`/`evidence`/`uncertainty` from the root into `bracket`, drops echoed prompt-input keys (`deck_id`/`deck_name`/`format`), translates `bracket.rationale` into `headline`/`tagline`, defaults missing axis `uncertainty` arrays, strips a stray `legality_review.legal` flag, coerces unknown restriction statuses to a safe value, and synthesizes a summary from the bracket headline/tagline when omitted. `friction_drivers`/`rule_zero_talking_points`/`recommendations` are now optional at the root. Prompt `deck-eval-v2` system message + response_contract include an explicit JSON skeleton and a "do not echo prompt input keys" rule so future runs hit the schema directly. The user-visible "AI evaluation failed: $.summary is required, $.bracket.restrictions is required, …" failure mode is gone.
